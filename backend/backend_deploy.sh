...
set -e
docker run -v /home/student/logsBackDocker:/app/logs -d --name backend --env=${SPRING_DATASOURCE_PASSWORD} --env=${SPRING_DATASOURCE_USERNAME} --env=${SPRING_DATA_MONGODB_URI} \
