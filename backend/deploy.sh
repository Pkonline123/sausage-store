...
(echo "PSQL_HOST=${PSQL_HOST}" && echo PSQL_PORT=${PSQL_PORT} && echo "PSQL_DBNAME=${PSQL_DBNAME}" && echo "PSQL_USER=${PSQL_USER}" && echo "PSQL_PASSWORD=${PSQL_PASSWORD}" && echo "MONGO_USER=${MONGO_USER}" && echo "MONGO_PASSWORD=${MONGO_PASSWORD}" && echo "MONGO_HOST=${MONGO_HOST}" && echo "MONGO_DATABASE=${MONGO_DATABASE}") > /etc/systemd/system/var-file-backend-sausage
