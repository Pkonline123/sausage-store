pipeline {
    ...
    stages {
        ...
        stage('Send POST') {
            steps {
                curl -X POST -H "Content-type: application/json" --data sh '{"message":"Андрей Зачитайлов собрал приложение."}' https://api.pachca.com/webhooks/01GHKAEEBHC027DJAH7CHPTVF1
            }
        }
    }
}