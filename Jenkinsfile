pipeline {
    ...
    stages {
        ...
        stage('Send POST') {
            steps {
              scurl -X POST -H "Content-type: application/json" --data '{"message":"Андрей Зачитайлов собрал приложение."}' https://api.pachca.com/webhooks/01GHKAEEBHC027DJAH7CHPTVF1
            }
        }
    }
}